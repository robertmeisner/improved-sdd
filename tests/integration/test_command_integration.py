...
from src.core.models import TemplateSource, TemplateSourceType, TemplateResolutionResult
...
class TestCommandIntegration:
    ...
    def test_init_command_with_real_services(self, runner, temp_project_dir, mock_template_source):
        ...
        with patch("src.utils.check_tool") as mock_check_tool:
            with patch("src.utils.offer_user_choice") as mock_offer_choice:
                with patch("src.services.template_resolver.TemplateResolver.resolve_templates_with_transparency") as mock_resolve:
                    # Setup mocks
                    mock_check_tool.return_value = True
                    mock_offer_choice.side_effect = ["python-cli", ["github-copilot"], True]  # choices
        ...
        with patch("src.utils.check_tool") as mock_check_tool:
            with patch("src.utils.offer_user_choice") as mock_offer_choice:
                with patch("src.services.template_resolver.TemplateResolver.resolve_templates_with_transparency") as mock_resolve:
                    # Setup mocks
                    mock_check_tool.return_value = True
                    mock_offer_choice.side_effect = ["python-cli", ["github-copilot"], True]
                    mock_resolve.return_value = TemplateResolutionResult(
                        source=mock_template_source,
                        success=True,
                        message="Templates resolved successfully",
    ...
    def test_delete_command_integration(self, runner, temp_project_dir):
        ...
        with patch("src.commands.delete.Confirm.ask") as mock_confirm:
    ...
    def test_check_command_missing_tools(self, runner):
        ...
        with patch("src.commands.check.check_tool") as mock_check_tool:
            with patch("src.commands.check.check_github_copilot") as mock_check_copilot:
                ...
                assert result.exit_code == 0
                # Command should complete but show missing tools
    ...
    async def test_async_service_integration(self, mock_template_source, temp_project_dir):
        ...
        from core.interfaces import FileTrackerProtocol, CacheManagerProtocol, GitHubDownloaderProtocol
        ...
        with patch("src.utils.check_tool") as mock_check_tool:
            with patch("src.utils.offer_user_choice") as mock_offer_choice:
                with patch("src.services.template_resolver.TemplateResolver.resolve_templates_with_transparency") as mock_resolve:
                    ...
                    mock_check_tool.return_value = True
                    mock_offer_choice.side_effect = ["python-cli", ["github-copilot"], True]
                    mock_resolve.return_value = mock_template_source
