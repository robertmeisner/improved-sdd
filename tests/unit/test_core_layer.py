...
from src.core import (  # Configuration; Exceptions; Models
    AI_TOOLS,
    APP_TYPES,
    BANNER,
    TAGLINE,
...
class TestConfiguration:
    ...
    def test_app_type_config_dataclass(self):
        ...
        app_config = AppTypeConfig(description="Test app type", instruction_files=["test1", "test2"])
...
class TestExceptions:
    ...
    def test_template_error_context_summary(self):
        ...
        error = TemplateError("Test error", operation="test_op", file_path=Path("test.txt"), context={"test": "value"})
    ...
    def test_network_error_inheritance_and_context(self):
        ...
        error = NetworkError(
            "Network connection failed", operation="download", url="https://example.com", status_code=500
        )
    ...
    def test_github_api_error_inheritance_and_context(self):
        ...
        error = GitHubAPIError(
            "API rate limit exceeded", status_code=403, api_endpoint="/repos/owner/repo", rate_limit_remaining=0
        )
    ...
    def test_rate_limit_error_inheritance_and_context(self):
        ...
        error = RateLimitError(retry_after=3600, rate_limit_reset=1672531200)  # Unix timestamp
    ...
    def test_timeout_error_inheritance_and_context(self):
        ...
        error = TimeoutError("Request timed out", timeout_seconds=30.0, url="https://api.github.com")
    ...
    def test_validation_error_inheritance_and_context(self):
        ...
        error = ValidationError(
            "Invalid template format", validation_type="format_check", expected_value="markdown", actual_value="text"
        )
    ...
    def test_exception_hierarchy_structure(self):
        ...
        for exc_class in [TemplateError, NetworkError, GitHubAPIError, RateLimitError, TimeoutError, ValidationError]:
...
class TestModels:
    ...
    def test_progress_info_dataclass(self):
        ...
        progress = ProgressInfo(
            phase="download", bytes_completed=1024, bytes_total=2048, percentage=50.0, speed_bps=1024, eta_seconds=1
        )
    ...
    def test_progress_info_optional_fields(self):
        ...
        progress = ProgressInfo(phase="extract", bytes_completed=512, bytes_total=1024, percentage=50.0)
    ...
    def test_template_source_dataclass(self):
        ...
        source = TemplateSource(path=source_path, source_type=TemplateSourceType.LOCAL, size_bytes=4096)
    ...
    def test_template_source_optional_size(self):
        ...
        source = TemplateSource(path=Path("/test"), source_type=TemplateSourceType.GITHUB)
    ...
    def test_template_resolution_result_dataclass(self):
        ...
        source = TemplateSource(path=Path("/local/templates"), source_type=TemplateSourceType.LOCAL)

        result = TemplateResolutionResult(
            source=source, success=True, message="Templates found locally", fallback_attempted=False
        )
    ...
    def test_template_resolution_result_properties(self):
        ...
        # Test local source
        local_source = TemplateSource(path=Path("/local"), source_type=TemplateSourceType.LOCAL)
        local_result = TemplateResolutionResult(source=local_source, success=True, message="Local")
        ...
        # Test bundled source
        bundled_source = TemplateSource(path=Path("/bundled"), source_type=TemplateSourceType.BUNDLED)
        bundled_result = TemplateResolutionResult(source=bundled_source, success=True, message="Bundled")
        ...
        # Test GitHub source
        github_source = TemplateSource(path=Path("/github"), source_type=TemplateSourceType.GITHUB)
        github_result = TemplateResolutionResult(source=github_source, success=True, message="GitHub")
    ...
    def test_template_resolution_result_no_source(self):
        ...
        result = TemplateResolutionResult(source=None, success=False, message="No templates found")
...
class TestCoreLayerIntegration:
    ...
    def test_configuration_with_models(self):
        ...
        source = TemplateSource(
            path=Path(f"/templates/{tool_config.template_dir}"), source_type=TemplateSourceType.BUNDLED
        )
    ...
    def test_exceptions_with_models(self):
        ...
        source = TemplateSource(path=Path("/test/templates"), source_type=TemplateSourceType.LOCAL)
    ...
    def test_core_layer_functionality_preservation(self):
        ...
        resolution = TemplateResolutionResult(
            source=TemplateSource(Path("/test"), TemplateSourceType.LOCAL), success=True, message="Found"
        )
