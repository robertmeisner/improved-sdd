...
class TestProgressTracker:
    ...
    def test_update_progress_from_info_with_bytes(self):
        ...
        progress_info = ProgressInfo(phase="download", bytes_completed=500, bytes_total=1000, percentage=50.0)

        self.tracker.update_progress_from_info(mock_progress, task_id, progress_info)

        mock_progress.update.assert_called_once_with(task_id, completed=500, total=1000, description="Downloading...")
    ...
    def test_update_progress_from_info_with_percentage_only(self):
        ...
        progress_info = ProgressInfo(phase="extract", bytes_completed=75, bytes_total=100, percentage=75.0)

        self.tracker.update_progress_from_info(mock_progress, task_id, progress_info)

        mock_progress.update.assert_called_once_with(task_id, completed=75, total=100, description="Extracting...")
    ...
    def test_update_progress_from_info_with_percentage_and_total(self):
        ...
        progress_info = ProgressInfo(phase="processing", bytes_completed=500, bytes_total=2000, percentage=25.0)

        self.tracker.update_progress_from_info(mock_progress, task_id, progress_info)

        mock_progress.update.assert_called_once_with(task_id, completed=500, total=2000, description="Processing...")
    ...
    def test_update_progress_from_info_indeterminate(self):
        ...
        progress_info = ProgressInfo(phase="initializing", bytes_completed=0, bytes_total=0, percentage=0.0)

        self.tracker.update_progress_from_info(mock_progress, task_id, progress_info)

        mock_progress.update.assert_called_once_with(task_id, description="Initializing...")
    ...
    def test_update_progress_from_info_no_phase(self):
        ...
        progress_info = ProgressInfo(phase="", bytes_completed=300, bytes_total=600, percentage=50.0)  # Empty phase

        self.tracker.update_progress_from_info(mock_progress, task_id, progress_info)

        mock_progress.update.assert_called_once_with(task_id, completed=300, total=600, description="Processing...")
    ...
    def test_create_callback_for_progress(self):
        ...
        progress_info = ProgressInfo(phase="test", bytes_completed=100, bytes_total=200, percentage=50.0)
        callback(progress_info)

        mock_progress.update.assert_called_once_with(task_id, completed=100, total=200, description="Testing...")
    ...
    def test_run_with_progress_download_type(self, mock_progress_class):
        ...
        result = self.tracker.run_with_progress(
            mock_operation, description="Downloading files...", progress_type="download"
        )

        assert result == "operation_result"
        mock_progress_instance.add_task.assert_called_once_with("Downloading files...", total=None)
        mock_progress_instance.update.assert_called_once_with(1, description="Complete!", completed=100, total=100)
    ...
    def test_run_with_progress_extraction_type(self, mock_progress_class):
        ...
        result = self.tracker.run_with_progress(mock_operation, description="Extracting...", progress_type="extraction")

        assert result == "extraction_result"
        mock_progress_instance.add_task.assert_called_once_with("Extracting...", total=None)
        mock_progress_instance.update.assert_called_once_with(2, description="Complete!", completed=100, total=100)
    ...
    def test_run_with_progress_generic_type(self, mock_progress_class):
        ...
        def mock_operation(callback):
            progress_info = ProgressInfo(phase="testing", bytes_completed=50, bytes_total=100, percentage=50.0)
            callback(progress_info)
    ...
    def test_run_with_progress_operation_with_callback(self):
        ...
        with patch("src.ui.progress.Progress", return_value=mock_progress_context):
            ...
            progress_info = ProgressInfo(phase="test", bytes_completed=25, bytes_total=100, percentage=25.0)
...
class TestProgressTrackerIntegration:
    ...
    def test_progress_callback_integration(self):
        ...
        with progress:
            ...
            progress_info = ProgressInfo(phase="test", bytes_completed=50, bytes_total=100, percentage=50.0)
