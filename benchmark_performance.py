    ...
    def measure_startup_time(self, num_runs: int = 10) -> Dict[str, float]:
        ...
        for i in range(num_runs):
            ...
            result = subprocess.run([
                sys.executable, "-c", 
                "import sys; sys.path.insert(0, '.'); import src.improved_sdd_cli"
            ], capture_output=True, text=False, cwd=self.cli_path.parent.parent,
...
def main():
    ...
    try:
        ...
        print(f"   CLI startup time: {benchmark.results['startup_time']['mean']:.3f}s (±{benchmark.results['startup_time']['std_dev']:.3f}s)")
        print(f"   Module import time: {benchmark.results['import_time']['mean']:.3f}s (±{benchmark.results['import_time']['std_dev']:.3f}s)")
