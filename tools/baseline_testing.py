...
# Import performance monitoring for integration
try:
    from performance_monitor import PerformanceMonitor
    PERFORMANCE_MONITORING_AVAILABLE = True
except ImportError:
    PERFORMANCE_MONITORING_AVAILABLE = False
    print("Warning: Performance monitoring not available - install performance_monitor.py")
...
class BaselineTestSuite:
    ...
    def run_command(self, args: List[str], cwd: Path = None, 
                   input_data: str = None, timeout: int = 30) -> CommandResult:
        ...
        try:
            ...
            return CommandResult(
                ...
                environment_snapshot={k: v for k, v in env.items() 
                                    if k.startswith(('PYTHON', 'PATH', 'IMPROVED_SDD'))}
            )
    ...
    def capture_init_command_behaviors(self) -> Dict[str, CommandResult]:
        ...
        scenarios = [
            # Basic scenarios
            (["init", "--help"], "init_help"),
            (["init", "test-project", "--new-dir", "--force", "--offline", "--app-type", "python-cli", "--ai-tools", "github-copilot"], "init_new_project_offline"),
            (["init", "--here", "--force", "--offline", "--app-type", "mcp-server", "--ai-tools", "github-copilot"], "init_here_offline"),
            
            # Error scenarios
            (["init"], "init_no_args_no_here"),  # Should error - no project name, no --here
            (["init", "--app-type", "invalid-type", "--force", "--offline", "--ai-tools", "github-copilot"], "init_invalid_app_type"),
            (["init", "--ai-tools", "invalid-tool", "--force", "--offline", "--app-type", "python-cli"], "init_invalid_ai_tool"),
        ]
    ...
    def compare_with_baseline(self, current_results: Dict[str, Any], 
                            baseline_file: str = "cli_baseline.json") -> Dict[str, Any]:
            ...
            if category in baseline and category in current_results:
                ...
                for test_name in baseline_behaviors:
                    ...
                    if baseline_result["exit_code"] != current_result_dict["exit_code"]:
                        differences.append(f"exit_code: {baseline_result['exit_code']} → {current_result_dict['exit_code']}")
        ...
        if "performance" in baseline and "performance" in current_results:
            ...
            # Startup time change
            startup_change = (current_perf["startup_time_ms"] - baseline_perf["startup_time_ms"]) / baseline_perf["startup_time_ms"] * 100
            ...
            if baseline_perf["memory_usage_mb"] > 0:
                memory_change = (current_perf["memory_usage_mb"] - baseline_perf["memory_usage_mb"]) / baseline_perf["memory_usage_mb"] * 100
